...
for (chr in 1:22) {
    ...
    tmp_snp <- snp_match(sumstats[sumstats$chr==chr,], map, match.min.prop = 0.0001)
...
# for CAD_2015 summary statistics download from https://github.com/privefl/paper-misspec/blob/main/code/prepare-sumstats/CAD.R
#h2_est <- 0.0661

# for CAD2021 with 1116999 snps h2 h2 estimated by LDSC h2 is 0.0673
# h2_est<-0.0673

# for CAD2021 with 1121634 snps, h2 estimated by ldpred2 is 0.0527527468850231, I use this to run all the finemap analysis
# h2_est <- 0.0528
...
beta_lassosum2 <- snp_lassosum2(corr, df_beta, ncores = NCORES,
        ...
        delta = signif(deltavalue, 2), nlambda = numlambda
    )
...
grid.param.name <-
     ...
     expand.grid(s = paste0('s:',signif(deltavalue, 2)),
                 nlambda = paste0('nl:',1:numlambda))
